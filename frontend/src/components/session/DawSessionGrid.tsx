...
export const DawSessionGrid: React.FC<DawSessionGridProps> = ({ project, fill = false }) => {
  ...
  return (
    <div className={`border border-white/10 rounded bg-[#15151a] overflow-hidden ${fill ? 'h-full flex flex-col' : ''}`}>
      ...
      <div className={`overflow-auto ${fill ? 'flex-1 min-h-0' : 'max-h-[420px]'}`}>
        <div
          className="grid min-w-[760px]"
          style={{
            gridTemplateColumns: `128px repeat(${tracks.length}, minmax(104px, 1fr)) 56px`,
          }}
        >
          ...
          <div className="bg-[#202027] border-b border-white/10 px-1 py-2 text-center text-[7px] font-mono uppercase tracking-wider text-zinc-500">
            Launch
          </div>
          ...
          {scenes.map((sceneName, sceneIndex) => (
            <React.Fragment key={`${sceneName}-${sceneIndex}`}>
              ...
              <button
                ...
                className={[
                  'border-b border-white/10 min-h-14 inline-flex flex-col items-center justify-center gap-0.5',
              ...
              >
                ...
                <span className="text-[7px] font-mono uppercase">Scene</span>
              </button>
            </React.Fragment>
          ))}
